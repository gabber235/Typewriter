...
@Entry(
    "witch_path_stream_display",
    "Displaying a path stream with witch particles",
    Colors.MYRTLE_GREEN,
    "game-icons:pointy-hat"
)
class WitchPathStreamDisplayEntry(
...
) : PathStreamDisplayEntry<WitchPathStreamDisplay> {
    override val klass: KClass<WitchPathStreamDisplay> get() = WitchPathStreamDisplay::class
    override fun createDisplay(
        ref: Ref<RoadNetworkEntry>,
        player: Player,
        startPosition: (Player) -> Position,
        endPosition: (Player) -> Position
    ): WitchPathStreamDisplay = WitchPathStreamDisplay(ref, player, startPosition, endPosition, refreshDuration, speed)
}

class WitchPathStreamDisplay(
    ref: Ref<RoadNetworkEntry>,
    player: Player,
    startPosition: (Player) -> Position,
    endPosition: (Player) -> Position,
    ...
    override fun displayPath() {
        lines.keys.retainAll { line ->
            ...
            WrapperPlayServerParticle(
                ...
                true,
                location.withY { it + 0 }.toPacketVector3d(),
