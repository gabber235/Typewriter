...
class LockHealthAudienceSpec : FunSpec() {
    ...
    init {
        ...
        test("a lethal hit on a locked player deals no damage") {
            val player = server.addPlayer()
            player.health = 1.0
            val display = LockHealthAudienceDisplay(ConstVar(1.0)).activeWith(player)

            val event = EntityDamageEvent(player, DamageCause.ENTITY_ATTACK, 6.0)
            display.onDamage(event)

            event.isCancelled shouldBe false
            event.finalDamage shouldBe (0.0 plusOrMinus 1e-9)
        }

        test("a hit the locked player survives lands in full") {
            val player = server.addPlayer()
            player.health = 20.0
            val display = LockHealthAudienceDisplay(ConstVar(20.0)).activeWith(player)

            val event = EntityDamageEvent(player, DamageCause.ENTITY_ATTACK, 6.0)
            display.onDamage(event)

            event.finalDamage shouldBe (6.0 plusOrMinus 1e-9)
        }

        test("the kill command still kills a locked player") {
            val player = server.addPlayer()
            player.health = 1.0
            val display = LockHealthAudienceDisplay(ConstVar(1.0)).activeWith(player)

            val event = EntityDamageEvent(player, DamageCause.KILL, Double.MAX_VALUE)
            display.onDamage(event)

            event.finalDamage shouldBe Double.MAX_VALUE
        }

        test("a lethal hit on a player outside the audience lands in full") {
            val member = server.addPlayer()
            val outsider = server.addPlayer()
            outsider.health = 1.0
            val display = LockHealthAudienceDisplay(ConstVar(1.0)).activeWith(member)

            val event = EntityDamageEvent(outsider, DamageCause.ENTITY_ATTACK, 6.0)
            display.onDamage(event)
