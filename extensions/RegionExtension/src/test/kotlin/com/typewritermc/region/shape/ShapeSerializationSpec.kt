...
class ShapeSerializationSpec : FunSpec({
    ...
    test("a shape read from json with dimensions no constructor accepts is refused") {
        val json = JsonParser.parseString("""{"case":"capsule_shape","value":{"radius":2.0,"halfHeight":-1.0}}""")

        val shape = gson.fromJson(json, Shape::class.java)

        shouldThrow<IllegalArgumentException> { shape.requireValid() }
    }

    test("an inline region holding a malformed shape builds no shape") {
        val json = JsonParser.parseString("""{"case":"cuboid_shape","value":{"halfX":2.0,"halfY":-3.0,"halfZ":2.0}}""")
        val inline = RegionDefinitionData(shape = gson.fromJson(json, Shape::class.java))

        inline.buildShapeOrNull().shouldBeNull()
    }
})
