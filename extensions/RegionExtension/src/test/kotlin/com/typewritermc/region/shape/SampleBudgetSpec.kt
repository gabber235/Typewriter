...
class SampleBudgetSpec : FunSpec({
    ...
    test("a diagonal band costs its own area rather than its bounding box's") {
        ...
        // a thousandth of its box, so it tests about as many cap points as a straight band of the
        // same area only when the walk stays inside the outline.
        val half = 7071.0
        val diagonal = PolygonShape(
            points = listOf(
        ...
        val diagonalWork = diagonal.capCandidates(0.5).count()
        val straightWork = straight.capCandidates(0.5).count()

        (diagonalWork <= 4 * straightWork.coerceAtLeast(1)) shouldBe true
    }
