...
class ConeShapeSpec : FunSpec({
    ...
    test("nearest outside lands on the boundary it measured to") {
        val narrow = ConeShape(length = 10.0, halfAngleDegrees = 5.0)
        narrow.nearestBoundaryPoint(Vector(0.0, 0.0, -2.0)) shouldBe Vector.ZERO
        val beside = ConeShape(length = 5.0, halfAngleDegrees = 30.0).nearestBoundaryPoint(Vector(10.0, 0.0, 0.0))
