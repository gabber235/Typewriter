...
abstract class RegionContentMode(context: ContentContext, player: Player) : ContentMode(context, player) {
    ...
    /**
     * The working model of the session: placement, marks and sizes. It is the editing truth
     * until the mode closes, the history records snapshots of it, and the staged writes are
     * derived from it. Replaced whole on every change, so the async tick always reads one
     * consistent snapshot.
     */
    @Volatile
    private var state: EditorState = EditorState.EMPTY

    @Volatile
    private var gesture: Gesture = Gesture.Idle

    private val workingOrigin: Position? get() = state[ORIGIN]
    private val workingOffset: Vector get() = state[OFFSET]
    private val workingYaw: Float? get() = state[YAW]
    private val workingPitch: Float? get() = state[PITCH]
    private val workingRoll: Float? get() = state[ROLL]
    private val resizeShift: Vector get() = state[RESIZE_SHIFT]
    ...
    private val carrying: Boolean get() = gesture is Gesture.RegionCarry
    private val grabbedFace: RegionFace? get() = (gesture as? Gesture.FaceDrag)?.face
    ...
    final override suspend fun setup(): Result<Unit> {
        ...
        bossBar {
            title = bossBarTitle()
            color = when {
                gestureActive() -> BossBar.Color.BLUE
    ...
    /**
     * Scroll input while the wand is held and no gesture runs. Return `true` to consume the
     * notches. Runs before the sneak gate, so a plain scroll can mean something to the wand.
     */
    protected open fun onWandScroll(steps: Int): Boolean = false

    /** Called after a history restore replaced part of the working model. */
    protected open fun onStateRestored() {}

    /** The mode's own values seeded into the working model, like sizes read from the entry. */
    internal open fun seedModeState(state: EditorState): EditorState = state
    ...
    /** Reads one value of the working model. */
    internal fun <T> read(field: EditField<T>): T = state[field]

    /**
     * Writes one value of the working model and stages the entry fields again, so the staged
     * writes always describe the model as it stands.
     */
    internal fun <T> write(field: EditField<T>, value: T) {
        state = state.with(field, value)
        restageWorkingFields()
    }
    ...
    protected fun updateWorkingOrigin(position: Position) = write(ORIGIN, position)

    /**
     * Stores the origin that puts the region's anchor at [anchor] in [world], facing nowhere.
     *
     ...
     *
     * The configured offset is taken back out, so the anchor lands where the marks put it, and
     * the resize shift is folded away for the fresh placement. Call it after the capture wrote
     * its rotation: the offset turns with the yaw.
     */
    protected fun placeCapturedAnchor(world: World, anchor: Vector) {
        write(RESIZE_SHIFT, Vector.ZERO)
        val origin = ResolvedTransform.originForAnchor(
            world,
            Vector(anchor.x.round(2), anchor.y.round(2), anchor.z.round(2)),
            workingOffset,
            workingYaw ?: 0f,
            rotatesWithOrigin(),
        )
        updateWorkingOrigin(origin.rounded())
    }

    protected fun updateWorkingYaw(yaw: Float) = write(YAW, yaw)

    /** Rotates the working pitch and stages it. */
    protected fun updateWorkingPitch(pitch: Float) = write(PITCH, pitch)

    /** Rotates the working roll and stages it. */
    protected fun updateWorkingRoll(roll: Float) = write(ROLL, roll)
    ...
    /**
     * Accumulates a placement shift along a local frame direction, rotated the way the region
     * resolves. Single sided face moves use this to keep the opposite face in place.
     * The shift lives next to the size values instead of in the working origin, so the
     * resize tool and the move tool never write the same undo history field and tool scoped
    ...
    protected fun shiftResizeAnchor(localDirection: Vector, distance: Double): Boolean {
        val transform = workingTransform() ?: return false
        val rotated = transform.rotateLocalToWorld(localDirection)
        val shift = resizeShift
        write(
            RESIZE_SHIFT,
            Vector(
                (shift.x + rotated.x * distance).round(3),
                (shift.y + rotated.y * distance).round(3),
                (shift.z + rotated.z * distance).round(3),
            ),
        )
        return true
    }

    private fun rotatesWithOrigin(): Boolean = definition()?.rotateWithOrigin == true

    /**
     * Stages the subclass's size fields from the working model through [restageField]. Staged
     * writes are derived state: they follow the working values instead of living in the undo
     * history themselves.
     */
    ...
    /** Puts [values] over the working model and stages the result, without any feedback. */
    private fun applyValues(values: EditorState) {
        state = state.overriddenBy(values)
        restageWorkingFields()
    }
     ...
     */
    private fun restoreValues(values: EditorState) {
        applyValues(values)
        restoreEpoch++
        val touched = values.fields
        when {
            ROLL in touched -> rotatePitchMode = true
            PITCH in touched && YAW !in touched -> rotatePitchMode = true
            YAW in touched -> rotatePitchMode = false
        }
        onStateRestored()
    }

    /** A short readout of the fields a restore moved, for the undo and redo feedback. */
    private fun describeRestore(values: EditorState): String? {
        val parts = mutableListOf<String>()
        val touched = values.fields
        if (ORIGIN in touched) {
            values[ORIGIN]?.let { parts += "origin ${it.x.round(2)}, ${it.y.round(2)}, ${it.z.round(2)}" }
        }
        if (YAW in touched) values[YAW]?.let { parts += "yaw ${it.toDouble().round(1)}°" }
        if (PITCH in touched) values[PITCH]?.let { parts += "pitch ${it.toDouble().round(1)}°" }
        if (ROLL in touched) values[ROLL]?.let { parts += "roll ${it.toDouble().round(1)}°" }
        for (field in touched) {
            val readout = field.readout ?: continue
            val value = values.valueOf(field) ?: continue
            val shown = (value as? Double)?.round(2) ?: value
            parts += "$readout $shown"
        }
     ...
     * included: the base cannot know which fields the gesture's entry will span, so no tool
     * is provably safe to interleave.
     */
    internal fun recorded(tool: EditTool, label: String, coalesceMillis: Long = 0, change: () -> Boolean): Boolean {
        val refusal = gestureRefusal(allowRotation = tool == EditTool.ROTATE)
        if (refusal != null) {
            refuse(refusal)
            return false
        }
        ...
        history.record(tool, label, before, state, coalesceMillis)
        return true
    }

    /**
     * Why the running gesture blocks another edit, or `null` when nothing blocks it. With
     * [allowRotation] a region carry lets the edit through, see [recorded].
     */
    private fun gestureRefusal(allowRotation: Boolean = false): String? = when (val current = gesture) {
        Gesture.Idle -> null
        is Gesture.FaceDrag -> "<red>Keep or put back the grabbed face first."
        is Gesture.RegionCarry -> if (allowRotation) null else "<red>Place the region down first."
        is Gesture.Mode -> current.owned.refusal
    }

    /** Whether any gesture holds the region, a face or a mode owned piece of it. */
    internal fun gestureActive(): Boolean = gesture != Gesture.Idle

    /** The mode owned gesture in progress, or `null` when none runs. */
    internal fun activeModeGesture(): ModeGesture? = (gesture as? Gesture.Mode)?.owned

    /**
     * Starts a mode owned gesture like the polygon's point carry. Refuses, with feedback, while
     * another gesture runs.
     */
    internal fun startModeGesture(owned: ModeGesture): Boolean {
        val refusal = gestureRefusal()
        if (refusal != null) {
            refuse(refusal)
            return false
        }
        gesture = Gesture.Mode(owned)
        return true
    }

    /** Ends [owned] without putting it back or recording it, for a gesture whose target vanished. */
    internal fun abandonModeGesture(owned: ModeGesture) {
        val current = gesture
        if (current is Gesture.Mode && current.owned === owned) gesture = Gesture.Idle
    }

    /** Puts back whatever gesture runs, restoring its base state. */
    internal fun cancelGesture() {
        when (val current = gesture) {
            Gesture.Idle -> {}
            is Gesture.RegionCarry -> cancelCarry()
            is Gesture.FaceDrag -> cancelFaceDrag()
            is Gesture.Mode -> {
                gesture = Gesture.Idle
                current.owned.cancel()
            }
        }
    }

    /** Keeps whatever gesture runs, recording it as one history entry. */
    private fun finishGesture() {
        when (val current = gesture) {
            Gesture.Idle -> {}
            is Gesture.RegionCarry -> placeCarry()
            is Gesture.FaceDrag -> confirmFaceDrag()
            is Gesture.Mode -> {
                gesture = Gesture.Idle
                current.owned.finish()
            }
        }
    }
    ...
    private fun applyToolResult(tool: EditTool, result: ToolHistoryResult, redo: Boolean) {
        when (result) {
     ...
     * A slot without a tool, like the teleport shard, owns no history and lands here too.
     */
    private fun nothingLeftForTool(tool: EditTool?, redo: Boolean) {
        val what = tool?.let { "the ${it.label} tool" } ?: "this tool"
        player.sendActionBar("<dark_gray>Nothing left to ${if (redo) "redo" else "undo"} for $what.".asMini())
    }

    /**
     * A newer change by another tool overlaps the fields this undo or redo would move, so
     ...
     */
    private fun entangledHint(blockingTool: EditTool, redo: Boolean) {
        player.sendActionBar(
            "<gray>Later ${blockingTool.label} changes overlap this. <white>Use the arrow</white> to ${if (redo) "redo" else "undo"} in order.".asMini(),
        )
    }
    ...
    private fun toolForSlot(slot: Int): EditTool? = when (slot) {
    ...
    private fun undoEverything() {
        ...
        restoreValues(restored.values)
        undoFeedback("Undid", restored.label, EditorState.EMPTY)
    }
    ...
    private fun redoEverything() {
        ...
        restoreValues(restored.values)
        undoFeedback("Redid", restored.label, EditorState.EMPTY, pitch = 1.3f)
    }

    private fun undoFeedback(verb: String, label: String, values: EditorState, pitch: Float = 0.7f) {
    ...
    private fun canApply(): Boolean = gestureActive() || stagedWrites.isNotEmpty()
    ...
    private fun sessionPreview(region: PendingRegion): SessionPreview = SessionPreview(
        ...
        shape = region.shape,
        color = editColor(),
        activity = when (val current = gesture) {
            Gesture.Idle -> "is editing"
            is Gesture.RegionCarry -> "is carrying the region"
            is Gesture.FaceDrag -> "is dragging the ${current.face.label}"
            is Gesture.Mode -> current.owned.activity
        },
    )
     ...
     * since it cannot resolve at edit time.
     */
    ...
    protected fun workingTransform(): ResolvedTransform? {
        ...
        return ResolvedTransform.resolve(
            origin,
            workingOffset,
            workingYaw ?: 0f,
            workingPitch ?: 0f,
    ...
    /** The working model as it stands, for a gesture to keep as its base. */
    internal fun snapshot(fields: Collection<EditField<*>>): EditorState = state.only(fields)

    /** Records a grab to place gesture as one history entry, like the region carry. */
    internal fun recordGestureEntry(tool: EditTool, label: String, before: EditorState): Boolean =
        history.record(tool, label, before, state.only(before.fields))

    /** Applies a gesture's base state back, snapping the next render. */
    internal fun restoreGestureBase(values: EditorState) = restoreValues(values)
    ...
    /**
     * Copies the constant placement fields and the mode's own values out of the entry, once.
     * Staged tool writes are not visible through the entry, so this working copy is the
     * editing truth until the mode closes. Variable bound fields stay `null` and their tools
     * refuse with a hint.
     */
    private fun seedWorkingState() {
        if (stateSeeded) return
        stateSeeded = true

        var seeded = EditorState.EMPTY
        val definition = definition()
        if (definition != null) {
            seeded = seeded
                .with(ORIGIN, (definition.origin as? ConstVar)?.value)
                .with(OFFSET, (definition.offset as? ConstVar)?.value ?: Vector.ZERO)
                .with(YAW, (definition.yaw as? ConstVar)?.value)
                .with(PITCH, (definition.pitch as? ConstVar)?.value)
                .with(ROLL, (definition.roll as? ConstVar)?.value)
        }
        state = seedModeState(seeded)
    ...
    private fun onScroll(heldSlot: Int, steps: Int): Boolean {
        val current = gesture
        if (current is Gesture.RegionCarry) {
            if (heldSlot != MOVE_SLOT) return false
            val distance = (current.distance + steps * CARRY_SCROLL_STEP)
                .coerceIn(CARRY_MIN_DISTANCE, CARRY_MAX_DISTANCE)
            gesture = current.copy(distance = distance)
            return true
        }
        if (current is Gesture.Mode && heldSlot == WAND_SLOT) {
            current.owned.scroll(steps)
            return true
        }
    ...
    private fun applyRotation(delta: Float, coalesce: Boolean) {
        ...
        if (rotatePitchMode) {
            ...
            recorded(EditTool.ROTATE, "Tilt", window) {
                val frameYaw = workingTransform()?.yawDegrees ?: yaw
                val (newPitch, newRoll) = steppedTilt(pitch, roll, tiltFacing(), frameYaw, delta)
    ...
    private fun grabFace() {
        ...
        gesture = Gesture.FaceDrag(
            ...
            localEye = transform.toLocal(Vector(eye.x, eye.y, eye.z)),
            lastDelta = 0.0,
        )
    ...
    private fun tickFaceDrag() {
        val drag = gesture as? Gesture.FaceDrag ?: return
        val spec = faceSpec() ?: return
        val base = drag.transform
        ...
        val center = base.toWorld(drag.face.center)
        val normal = base.rotateLocalToWorld(drag.face.normal)
        ...
        gesture = drag.copy(lastDelta = delta)

        applyValues(drag.base)
        val result = spec.moveFace(drag.face, delta)
        if (result.changed) {
    ...
    private fun confirmFaceDrag() {
        ...
        if (!history.record(EditTool.RESIZE, drag.face.label, drag.base, state)) {
            player.sendActionBar("<gray>The ${drag.face.label} ended up where it started.".asMini())
            return
        }
        player.playSound(player.location, Sound.BLOCK_PISTON_CONTRACT, 0.5f, 1.1f)
        player.sendActionBar("<gold>Kept the ${drag.face.label}.".asMini())
        flashHeldItem()
    }
    ...
    private fun grabRegion() {
        ...
        val distance = player.eyeLocation.distance(Location(player.world, anchor.x, anchor.y, anchor.z))
            .coerceIn(CARRY_MIN_DISTANCE, CARRY_MAX_DISTANCE)
        ...
        gesture = Gesture.RegionCarry(base = state.only(listOf(ORIGIN)), distance = distance)
    ...
    private fun placeCarry() {
        val carry = gesture as? Gesture.RegionCarry ?: return
        gesture = Gesture.Idle
        val origin = workingOrigin ?: return
        val placed = origin.rounded()
        updateWorkingOrigin(placed)
        history.record(EditTool.MOVE, "Region carry", carry.base, state.only(carry.base.fields))
     ...
     * portal, or dying.
     */
    private fun releaseGestures() = cancelGesture()

    private fun cancelCarry() {
        val carry = gesture as? Gesture.RegionCarry ?: return
        gesture = Gesture.Idle
        restoreValues(carry.base)
     ...
     * view ray hits, or floating at the carry distance. The crosshair places the region's
     * visible anchor, so the stored origin is the one whose anchor lands there once the
     * offset and the resize shift are applied. Runs on the main thread because the ray trace
     * reads world state.
     */
    private fun tickCarry() {
        ...
        val hit = player.world
            .rayTraceBlocks(eye, direction, carry.distance, FluidCollisionMode.NEVER, true)
            ?.hitPosition
        val target = Vector(
            hit?.x ?: (eye.x + direction.x * carry.distance),
            hit?.y ?: (eye.y + direction.y * carry.distance),
            hit?.z ?: (eye.z + direction.z * carry.distance),
        )
        val snap = !player.isSneaking
        fun settle(value: Double): Double = if (snap) snapToHalfGrid(value) else value.round(2)
        val shift = resizeShift
        val anchor = Vector(settle(target.x) - shift.x, settle(target.y) - shift.y, settle(target.z) - shift.z)
        state = state.with(
            ORIGIN,
            ResolvedTransform.originForAnchor(
                origin.world,
                anchor,
                workingOffset,
                workingYaw ?: 0f,
    ...
    private fun updateOutline() {
        ...
        val spec = faceSpec()
        if (spec == null || player.inventory.heldItemSlot != resizeSlot || carrying || activeModeGesture() != null) {
            targetedFace = null
            pinnedFaceId = null
        ...
        val face = if (grabbed == null) {
            ...
            val dragEye = (gesture as? Gesture.FaceDrag)?.localEye ?: localEye
            spec.faces(dragEye).firstOrNull { it.id == grabbed.id } ?: grabbed
        }
    ...
    private fun updateToolGuides() {
        val transform = if (gestureActive()) null else workingTransform()
    ...
    private fun guideAnchor(transform: ResolvedTransform): Pair<Vector, Boolean> {
        ...
        val boundary = workingShape()?.takeIf { it.usable }?.nearestBoundaryPoint(transform.toLocal(eye)) ?: return anchor to true
    ...
    private fun nearestBoundaryOffset(transform: ResolvedTransform, eye: Vector): Vector? {
        val local = workingShape()?.takeIf { it.usable }?.nearestBoundaryPoint(transform.toLocal(eye)) ?: return null
    ...
    companion object {
        ...
        private val ORIGIN = EditField<Position?>("work.origin", null)
        private val OFFSET = EditField("work.offset", Vector.ZERO)
        private val YAW = EditField<Float?>("work.yaw", null)
        private val PITCH = EditField<Float?>("work.pitch", null)
        private val ROLL = EditField<Float?>("work.roll", null)
        private val RESIZE_SHIFT = EditField("size.shift", Vector.ZERO)
...
/**
 * What the builder is holding while an edit is in progress. Only one runs at a time, and
 * every other tool refuses while one does, see [RegionContentMode.recorded].
 */
private sealed interface Gesture {
    data object Idle : Gesture

    /** The region hangs on the crosshair; [base] holds the origin it goes back to. */
    data class RegionCarry(val base: EditorState, val distance: Double) : Gesture

    /**
     * A grabbed face follows the view. [transform] and [localEye] are frozen at the grab, so
     * the drag is measured against the region as it was, and [base] is the model it restarts
     * from on every tick.
     */
    data class FaceDrag(
        val face: RegionFace,
    ...
    /** A gesture a mode owns, like the polygon's point carry. */
    class Mode(val owned: ModeGesture) : Gesture
}

/**
 * A gesture a shape editor runs on top of the shared tools, started with
 * [RegionContentMode.startModeGesture]. The base editor drives it: it ticks it, hands it the
 * wand's scroll notches, and cancels or finishes it wherever it would a region carry.
 */
internal interface ModeGesture {
    /** The refusal every other tool answers with while this gesture runs. */
    val refusal: String

    /** What spectating players are told the editor is doing, like "is moving a point". */
    val activity: String

    /** Called every tick on the main thread, before the outline update. */
    fun tick()

    fun scroll(steps: Int)

    /** Puts the gesture back without keeping it. The gesture has already ended. */
    fun cancel()

    /** Keeps the gesture, recording it. The gesture has already ended. */
    fun finish()
}

private fun Position.rounded(): Position = Position(world, x.round(3), y.round(3), z.round(3), yaw, pitch)
