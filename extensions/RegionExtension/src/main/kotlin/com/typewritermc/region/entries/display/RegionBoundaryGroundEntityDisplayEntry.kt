...
class RegionBoundaryGroundEntityDisplay(
    ...
    override fun onDisplayPlayerRemoved(player: Player) {
        lines.remove(player.uniqueId)?.entities?.dispose()
    ...
    override fun renderForPlayer(player: Player, tracker: RegionTracker, transform: ResolvedTransform) {
        ...
        state.entities.update(player, samples, nearWindow(player))
    }
    ...
    /**
     * One player's ground line: the samples it was last placed at and the entities standing
     * there. On the static path a sample's index is ordered by angle around the region, so
     * unchanged terrain keeps every index at the same spot and resampling sends no packets. On
     * the animated path the index is a fixed slot on the loop instead.
     */
    private class PlayerGroundLine(definition: EntityDefinitionEntry) {
    ...
    private class GroundSample(
        ...
        private val position: Vector = point.position
        ...
        override fun positionProperty() = PositionProperty(world, position.x, position.y, position.z, yaw, 0f)
    }
}
