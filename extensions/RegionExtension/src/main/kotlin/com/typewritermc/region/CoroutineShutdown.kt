 ...
 * Waiting without a bound would hang the shutdown in that case; the bound leaves a warning
 * naming [owner] in the log instead.
 *
 * Returns `false` when the job was still running when the bound ran out.
 */
internal suspend fun Job.cancelAndJoinBounded(owner: String, timeout: Duration = SHUTDOWN_TIMEOUT): Boolean {
    cancel()
    val finished = withTimeoutOrNull(timeout) { join() } != null
    if (!finished) {
        logger.warning("$owner did not finish its cleanup within $timeout of being cancelled.")
    }
    return finished
}
