...
sealed interface Shape {
    ...
    /**
     * Throws when the fields describe no shape at all, like a negative radius.
     *
     * Every shape checks this when it is constructed. A shape read from json never is: the
     * serializer fills its fields in after allocating it, so whoever reads one from json has
     * to ask again.
     */
    fun requireValid()
    ...
    /**
     * The closest point on the shape's boundary, from inside and outside alike. A shape that is
     * not [usable] has no boundary, and answers with a point at least as far away as its
     * [signedDistance] says the boundary is.
     */
    fun nearestBoundaryPoint(localLoc: Vector): Vector
