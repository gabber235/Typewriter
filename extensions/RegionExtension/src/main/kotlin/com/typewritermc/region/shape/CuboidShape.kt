...
data class CuboidShape(
    ...
    override fun nearestBoundaryPoint(localLoc: Vector): Vector {
        ...
        val gapX = halfX - abs(localLoc.x)
        val gapY = halfY - abs(localLoc.y)
        val gapZ = halfZ - abs(localLoc.z)
        return when (min(gapX, min(gapY, gapZ))) {
            gapX -> Vector(if (localLoc.x >= 0) halfX else -halfX, localLoc.y, localLoc.z)
            gapY -> Vector(localLoc.x, if (localLoc.y >= 0) halfY else -halfY, localLoc.z)
