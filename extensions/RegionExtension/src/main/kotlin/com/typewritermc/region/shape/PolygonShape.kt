...
data class PolygonShape(
    ...
    /** Fewer than three points is left to [usable], so an outline still being drawn loads. */
    override fun requireValid() {
        require(halfHeight >= 0.0) { "Polygon half-height must be non-negative, was $halfHeight" }
    }
    ...
    override fun nearestBoundaryPoint(localLoc: Vector): Vector {
    ...
    private fun outsideDirection(localLoc: Vector): Vector? {
        ...
        val offset = localLoc - nearestBoundaryPoint(localLoc)
    ...
    override fun sampleBoundary(density: Double): Sequence<Vector> {
        ...
        return sequence {
            ...
            for (candidate in capCandidatesAtStep(step)) {
                if (closestBoundary2d(candidate.x, candidate.z).signedDistance >= -CAP_INSET) continue
                yield(Vector(candidate.x, -halfHeight, candidate.z))
                if (halfHeight > 0.0) yield(Vector(candidate.x, halfHeight, candidate.z))
            }
        }.withinSampleBudget()
    }

    /**
     * The cap grid points [sampleBoundary] tests at [density], before it drops the ones too
     * close to the outline. Internal so a test can bound the work the cap fill does.
     */
    internal fun capCandidates(density: Double): Sequence<Vector> {
        if (degenerate) return emptySequence()
        return capCandidatesAtStep(sampleStep(density, surfaceArea()))
    }

    /** The grid points at [step] spacing that lie inside the outline's row spans, at Y zero. */
    private fun capCandidatesAtStep(step: Double): Sequence<Vector> = sequence {
