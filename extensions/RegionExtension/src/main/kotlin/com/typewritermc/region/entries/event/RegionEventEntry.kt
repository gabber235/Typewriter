...
/**
 * Triggers this entry's pipeline for [player] when [cause] is one of its causes, and reports
 * whether the underlying Bukkit event should cancel.
 */
internal fun RegionEventEntry.dispatch(player: Player, cause: CrossingCause): Boolean {
    if (!causes.matches(cause)) return false
    val entries = listOf(this)
    entries.triggerAllFor(player, context())
    return entries.shouldCancel(player)
}
