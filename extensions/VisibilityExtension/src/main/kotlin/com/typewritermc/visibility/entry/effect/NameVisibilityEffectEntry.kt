...
class NameVisibilityEffectEntry(
    ...
    @Placeholder
    @Help(
        "The name the viewer reads on the target. Leave this empty to keep their real name. " +
            "Placeholders resolve against the target."
    )
    ...
    override val selfView: SelfView get() = SelfView.Never
...
private class NameVisibilityEffector(
    ...
    @Volatile
    private var nameContributionKey = -1
    ...
    override suspend fun initialize() {
        Dispatchers.Sync.switchContext {
            ...
            if (prefixText != null || suffixText != null) {
                nameContributionKey = teamManager.contribute(
                    viewer,
                    target,
    ...
    override fun onMetadata(packet: WrapperPlayServerEntityMetadata) {
        ...
        val alreadyOurs = metadata.readCustomName()?.orElse(null) == override &&
                metadata.readCustomNameVisible() == true
        if (!alreadyOurs) {
            lastServerName = metadata.readCustomName()?.orElse(null)
            lastServerNameSeen = true
            lastServerVisible = metadata.readCustomNameVisible() ?: false
        }
        @Suppress("UNCHECKED_CAST")
        (metadata as MutableList<EntityData<*>>).rewriteCustomName(override)
    }
    ...
    override suspend fun dispose() {
        ...
        Dispatchers.Sync.switchContext {
            ...
            // A bundled disguise keys the prefix and suffix by its fake, while the name stays on the target.
            if (nameContributionKey != -1 && nameContributionKey != targetEntityId) {
                teamManager.withdraw(rule.viewer, nameContributionKey, TeamContributionKind.NAME)
            }
