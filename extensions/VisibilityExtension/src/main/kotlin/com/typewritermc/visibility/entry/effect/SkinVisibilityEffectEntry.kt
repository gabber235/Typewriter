...
class SkinVisibilityEffectEntry(
    ...
    @Help(
        "Also apply this effect to the target's own view of themselves. " +
            "Their client respawns in place to show it, which closes an open inventory."
    )
    val self: Var<Boolean> = ConstVar(false),
) : VisibilityEffectEntry {
    override val selfView: SelfView get() = selfToggle(id, self)
