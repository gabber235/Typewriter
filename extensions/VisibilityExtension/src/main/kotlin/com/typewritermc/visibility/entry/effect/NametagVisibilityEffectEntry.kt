...
private class NametagVisibilityEffector(
    ...
    @Volatile
    private var hiddenNametagKey = -1
    ...
    override suspend fun initialize() {
        Dispatchers.Sync.switchContext {
            ...
            if (modification.hidesRealNametag) {
                hiddenNametagKey = teamManager.contribute(
                    viewer,
                    target,
    ...
    override suspend fun dispose() {
        overlay.detach(this@NametagVisibilityEffector, restore = { _, _, _ -> })
        Dispatchers.Sync.switchContext {
            val key = hiddenNametagKey
            if (key != -1) teamManager.withdraw(rule.viewer, key, TeamContributionKind.NAMETAG_HIDDEN)
        }
...
private val NametagModification.hidesRealNametag: Boolean
    get() = when (this) {
        is HiddenNametag -> true
        is ReplacedNametag -> true
        is ExtraLineNametag -> false
    }
