...
/**
 * An effect that cannot apply because of how its entries are configured, like a reference to an entry
 * that does not exist. The engine logs the message alone, which has to name the entry to fix.
 */
class VisibilityConfigurationException(message: String) : IllegalStateException(message)
