...
class VisibilityHideRegistry : Initializable, KoinComponent {
    ...
    /**
     * Hides [target] from [viewer] and records what the viewer sees instead.
     *
     * A player is never hidden from themselves, but their own view still records the replacement, so the
     * effects bundled with a disguise they see on themselves follow the disguise.
     */
     ...
     */
    fun replacementFor(viewerId: UUID, targetId: UUID): VisibleReplacement? = replacements[viewerId to targetId]
