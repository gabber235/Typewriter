 ...
 *
 * [effectRefs] is the flattened tree of a bundle, see [MultipleVisibilityEffectEntry.leafEffects].
...
class MultipleVisibilityEffector(
    ...
    // Latched rather than read from the sub effectors, since the engine reads it again after disposal released them.
    @Volatile
    override var needsPairRerender: Boolean = false
        private set

    override suspend fun initialize() {
        if (effectRefs.isEmpty()) {
        ...
        try {
            created.forEach { effector ->
                ...
                try {
                    ...
                    if (effector.needsPairRerender) needsPairRerender = true
                }
            }
        } catch (e: Throwable) {
    ...
    override suspend fun dispose() {
        ...
        effectors.asReversed().forEach { effector ->
            try {
                ...
                logger.log(Level.SEVERE, "Failed to dispose sub effector for pair (${rule.viewer}, ${rule.target})", e)
            }
        }
        effectors.clear()
    }
    ...
    private fun createAllEffectors(refs: List<Ref<VisibilityEffectEntry>>): List<VisibilityEffector> = refs.map { ref ->
        val entry = ref.get() ?: throw VisibilityConfigurationException(
...
/**
 * The sub effects of a bundle that apply to a viewer looking at themselves.
 *
 * A bundle gets the self pair when any of its sub effects applies, and that says nothing about the
 * sub effects that declined. Without checking each of them again a bundle would apply an effect to a
 * viewer who disabled it for themselves. A ref that resolves to nothing is kept so that creating it
 * reports the entry as missing rather than silently dropping it.
 */
internal fun selfApplicableRefs(
    ...
    entry.selfView.partsFor(viewer).isNotEmpty()
}
