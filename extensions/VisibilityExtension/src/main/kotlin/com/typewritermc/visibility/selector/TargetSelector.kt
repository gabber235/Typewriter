...
sealed interface TargetSelector {
    /** How a ruler resolves this selector. Every selector is one of the kinds and returns itself. */
    val resolution: TargetResolution
}

/**
 * How a [TargetSelector] resolves, which decides how often a ruler has to ask it.
 */
sealed interface TargetResolution {
    /** Resolves to the same players for every viewer, so a ruler asks once per tick. */
    interface Shared : TargetResolution {
        fun resolve(): Set<UUID>
    }

    /** Resolves relative to a viewer, so a ruler asks once per viewer per tick. */
    interface PerViewer : TargetResolution {
        /**
         ...
         */
        fun resolveFor(viewer: Player, previous: Set<UUID>): Set<UUID>
    }
}
...
) : TargetSelector, TargetResolution.Shared {
    override val resolution: TargetResolution get() = this
...
class EveryoneTargetSelector : TargetSelector, TargetResolution.Shared {
    override val resolution: TargetResolution get() = this
...
) : TargetSelector, TargetResolution.PerViewer {
    override val resolution: TargetResolution get() = this
