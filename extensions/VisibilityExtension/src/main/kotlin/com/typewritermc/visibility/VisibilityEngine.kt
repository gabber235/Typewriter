...
class VisibilityEngine : Initializable, KoinComponent {
    ...
    override suspend fun initialize() {
        ...
        providers.forEach { provider ->
            try {
                ...
                logger.log(Level.SEVERE, "Failed to create visibility ruler from entry '${provider.id}'", e)
            }
        }
    ...
    override suspend fun shutdown() {
        ...
        rulers.forEach { ruler ->
            try {
                ...
                logger.log(Level.SEVERE, "Failed to dispose visibility ruler ${ruler::class.simpleName}", e)
            }
        }
        rulers = emptyList()
    ...
    /**
     * Disposes the effect [rule] drives and creates it again from the same rule.
     *
     * For an effector whose rendering can no longer follow the world, like a disguise whose viewer
     * changed world. A fresh effector is also what gives the siblings in a bundle the new entity to
     * follow. Does nothing once another rule took over the pair. Callable from any thread, the work
     * runs in the engine's scope.
     */
    fun restartEffectOf(rule: VisibilityRule) {
        scope.launch { restartEffectNow(rule) }
    }

    internal suspend fun restartEffectNow(rule: VisibilityRule) = mutex.withLock {
        if (shuttingDown) return@withLock
        val current = effectsByViewer[rule.viewer]?.get(rule.target) ?: return@withLock
        if (current.rule !== rule) return@withLock
        swapEffect(rule.pair, current, current.rule, current.exhaustedRules)
    }
        ...
        while (currentCoroutineContext().isActive) {
            ...
            val time = measureTime {
                ...
                try {
                    ...
                    logger.log(Level.SEVERE, "Visibility tick failed", e)
                }
            }
    ...
    private suspend fun guarded(what: () -> String, block: suspend () -> Unit) {
        try {
            ...
            logger.log(Level.SEVERE, "Failed to ${what()}", e)
        }
    }
    ...
    private fun createEffect(rule: VisibilityRule, exhausted: Set<VisibilityRule>): ActiveEffect? {
        ...
        if (effectEntry == null) {
            logger.warning(
                "Could not find visibility effect entry '${rule.effect.id}' referenced by entry '${rule.entryId}'"
            )
            return null
        }
    ...
    private suspend fun disposeEffect(pair: PlayerPair, effect: ActiveEffect) {
        ...
        withContext(NonCancellable) {
            try {
                ...
                logger.log(
                    Level.SEVERE,
                    "Failed to dispose visibility effector for pair (${pair.viewer}, ${pair.target})",
                    e,
                )
            }
        }
    ...
    /**
     * Logs an effector failure without a stack trace when it reports a broken configuration, whose
     * message already names the entry to fix. Every other failure keeps its cause.
     */
    private fun logEffectFailure(message: String, e: Exception) {
        if (e is VisibilityConfigurationException) logger.severe(message)
        else logger.log(Level.SEVERE, message, e)
    }
    ...
    private fun refresh(fact: ReadableFactEntry, player: Player) {
        try {
            ...
            logger.log(Level.SEVERE, "Failed to refresh visibility count fact '${fact.id}' for ${player.name}", e)
        }
    }
