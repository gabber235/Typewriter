 ...
 *
 * Two kinds of packet are watched for every viewer, hooks or not, because rebuilding a player's own
 * tab list entry needs what they carry and the api does not expose it: an entity event is peeked
 * for the player's permission level, and a player info packet that can set up the player's own
 * chat session is decoded. Both are kept on the connection rather than in the bridge: a Typewriter
 * reload replaces this extension's classes while players stay connected, so a map in the bridge
 * would be empty for everyone already online. For the same reason the kept values only use types
 * this extension does not load, since an instance of a class loaded before a reload does not cast to
 * that class once it is loaded again.
 */
@Singleton
class VisibilityPacketBridge : PacketListenerAbstract(PacketListenerPriority.NORMAL), Initializable,
    ...
    /**
     * The chat session the server last announced to the connection's own player. Empty when it
     * announced that the player has none, and null when the bridge saw no announcement on this
     * connection.
     */
    fun ownChatSession(user: User): Optional<RemoteChatSession>? = ConnectionRecords.chatSession(user)

    /** The permission level the server last sent the connection's own player, or null when the bridge saw none. */
    fun ownPermissionLevel(user: User): Int? = ConnectionRecords.permissionLevel(user)
    ...
    override fun onPacketSend(event: PacketSendEvent?) {
        ...
        when (event.packetType) {
            ...
            PacketType.Play.Server.ENTITY_STATUS -> recordOwnPermissionLevel(event)
    ...
    /**
     * Records the permission level the server sends the receiving player about themselves. A respawn
     * resets it on the client, and the api does not expose the level to send it again.
     */
    private fun recordOwnPermissionLevel(event: PacketSendEvent) {
        val user = event.user
        val level = peek(event) { _, nextByte -> decodeOwnPermissionLevel(user.entityId, nextByte) } ?: return
        ConnectionRecords.recordPermissionLevel(user, level)
    }
    ...
    private fun handlePlayerInfo(viewerId: UUID, event: PacketSendEvent) {
        ...
        val byProfile = profileHooks[viewerId]?.takeIf { it.isNotEmpty() && actions and PROFILE_HOOK_ACTIONS != 0 }
        // A client receives its own chat session when it joins and whenever the session changes, and
        // rebuilding its own tab list entry later cannot do without it.
        val carriesOwnSession = actions and INITIALIZE_CHAT_ACTION != 0 &&
                peek(event) { _, nextByte -> mayCarryEntryOf(viewerId, nextByte) } == true
        if (byProfile == null && !carriesOwnSession) return

        val packet = WrapperPlayServerPlayerInfoUpdate(event)
        for (entry in packet.entries) {
            if (carriesOwnSession && entry.profileId == viewerId) {
                ConnectionRecords.recordChatSession(event.user, entry.chatSession)
            }
            val hooks = byProfile?.get(entry.profileId) ?: continue
    ...
    /**
     * What the bridge recorded about a connection's own player, stored on the connection itself.
     * Kept in its own object so netty is only loaded once a record is used: the unit tests load this
     * file without netty on the classpath.
     */
    private object ConnectionRecords {
        private val chatSessionKey: AttributeKey<Optional<RemoteChatSession>> =
            AttributeKey.valueOf("typewriter_visibility_own_chat_session")
        private val permissionLevelKey: AttributeKey<Int> =
            AttributeKey.valueOf("typewriter_visibility_own_permission_level")

        fun chatSession(user: User): Optional<RemoteChatSession>? = channelOf(user)?.attr(chatSessionKey)?.get()

        fun recordChatSession(user: User, session: RemoteChatSession?) {
            channelOf(user)?.attr(chatSessionKey)?.set(Optional.ofNullable(session))
        }

        fun permissionLevel(user: User): Int? = channelOf(user)?.attr(permissionLevelKey)?.get()

        fun recordPermissionLevel(user: User, level: Int) {
            channelOf(user)?.attr(permissionLevelKey)?.set(level)
        }

        private fun channelOf(user: User): Channel? = user.channel as? Channel
    }
}
...
/**
 * Whether a player info packet can hold the entry of [profileId], read off the start of the packet.
 * A packet about a single player names them right after the entry count. A longer list only tells
 * once it is decoded.
 */
internal fun mayCarryEntryOf(profileId: UUID, nextByte: () -> Byte?): Boolean {
    // The action bits come first.
    nextByte() ?: return false
    val count = decodeVarInt(nextByte) ?: return false
    if (count != 1) return count > 1
    val mostSignificantBits = decodeBigEndian(Long.SIZE_BYTES, nextByte) ?: return false
    val leastSignificantBits = decodeBigEndian(Long.SIZE_BYTES, nextByte) ?: return false
    return mostSignificantBits == profileId.mostSignificantBits &&
            leastSignificantBits == profileId.leastSignificantBits
}

/**
 * The permission level an entity event gives the player receiving it, or null for any other event.
 * The server sends the level as an event about the player's own entity, one event id per level.
 */
internal fun decodeOwnPermissionLevel(ownEntityId: Int, nextByte: () -> Byte?): Int? {
    val entityId = decodeBigEndian(Int.SIZE_BYTES, nextByte)?.toInt() ?: return null
    if (entityId != ownEntityId) return null
    val eventId = nextByte()?.toInt() ?: return null
    if (eventId !in PERMISSION_LEVEL_EVENT_IDS) return null
    return eventId - PERMISSION_LEVEL_EVENT_IDS.first
}

private fun decodeBigEndian(byteCount: Int, nextByte: () -> Byte?): Long? {
    var value = 0L
    repeat(byteCount) {
        val current = nextByte() ?: return null
        value = (value shl 8) or (current.toLong() and 0xFF)
    }
    return value
}

/** The entity event ids that give a player permission level 0 through 4, in order. */
internal val PERMISSION_LEVEL_EVENT_IDS = 24..28
...
/** The action bit, INITIALIZE_CHAT, of the player info packets that carry a chat session. */
private const val INITIALIZE_CHAT_ACTION = 0b10
