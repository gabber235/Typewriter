 ...
 *
 * @param followsReplacement false keeps the overlay on the target's own entity while a disguise shows something
 * else in its place.
 */
internal class EntityOverlay(
    private val rule: VisibilityRule,
    private val followsReplacement: Boolean = true,
) : KoinComponent {
     ...
     * either of them is offline or the overlay was already detached. The hook follows a disguise
     * replacement recorded before this runs, and [apply] receives the hooked id so immediate sends
     * address the visible entity. The hook is registered before [apply] runs, and [detach] removes it
     * even when [apply] threw.
     */
    suspend fun attach(
        ...
        Dispatchers.Sync.switchContext {
            ...
            val replacement =
                if (followsReplacement) hideRegistry.replacementFor(viewer.uniqueId, target.uniqueId) else null
            val entityId = replacement?.entityId ?: target.entityId
     ...
     *
     * Safe to call whether or not [attach] ran or completed, and more than once: only the first call
     * after a registration does anything.
     */
    suspend fun detach(
        hook: EntityPacketHook,
 ...
 */
internal class EntityFlagOverlay(
    rule: VisibilityRule,
    private val flag: Int,
    followsReplacement: Boolean = true,
) : EntityPacketHook {
    private val overlay = EntityOverlay(rule, followsReplacement)
    ...
    suspend fun attach(apply: (viewer: Player, target: Player) -> Unit = { _, _ -> }) {
        overlay.attach(this) { viewer, target, entityId ->
            lastServerFlags = target.serverSideEntityFlags()
            apply(viewer, target)
    ...
    suspend fun detach(release: (entityId: Int) -> Unit = {}) {
        overlay.detach(this, release) { viewer, target, entityId ->
            entityFlagsPacket(entityId, target.serverSideEntityFlags()) sendPacketTo viewer
        }
    }
    ...
    override fun onMetadata(packet: WrapperPlayServerEntityMetadata) {
        ...
        packet.entityMetadata.readEntityFlags()?.let { lastServerFlags = it and flag.inv() }
        packet.entityMetadata.rewriteEntityFlags(setMask = flag)
    }
