...
class VisibilityEngineSpec : FunSpec({
    ...
    afterTest {
        // Shut down while its dependencies are still registered, the order the plugin tears down in.
        engine.shutdown()
        stopKoin()
    }
    ...
    test("a shutdown while the plugin is already disabled waits for every effect to dispose") {
        val ruler = TestRuler(priority = 10)
        // One disposal scheduled onto the engine's own pool would still be running when shutdown
        // returns, and on a disabled plugin nothing would finish it. The gate holds the disposal open
        // until the shutdown is seen waiting for it.
        val gate = DisposeGate()
        val ghost = RecordingEffectEntry("ghost", events, disposeGate = gate)
        ...
        val shutdown = async(Dispatchers.Default) { engine.shutdown() }
        gate.started.await()
    ...
    test("a restart disposes the effect and creates it again from the same rule") {
        val ruler = TestRuler(priority = 10)
        ruler.apply(viewer, target, effect("disguise"))
        engine.awaitLifecycles()
        val rule = engine.activeRuleFor(viewer, target).shouldNotBeNull()
        events.clear()

        engine.restartEffectNow(rule)
        engine.awaitLifecycles()

        events shouldContainExactly listOf("disguise:dispose", "disguise:initialize")
        engine.activeRuleFor(viewer, target) shouldBeSameInstanceAs rule
    }

    test("a restart of the player's own self pair runs once") {
        val ruler = TestRuler(priority = 10)
        ruler.apply(viewer, viewer, RecordingEffectEntry("self", events, self = true))
        engine.awaitLifecycles()
        val rule = engine.activeRuleFor(viewer, viewer).shouldNotBeNull()
        events.clear()

        engine.restartEffectNow(rule)
        engine.awaitLifecycles()

        events shouldContainExactly listOf("self:dispose", "self:initialize")
    }

    test("a restart keeps the active rule and does not promote the rule it shadows") {
        val high = TestRuler(priority = 20)
        val low = TestRuler(priority = 10)
        high.apply(viewer, target, effect("high"))
        low.apply(viewer, target, effect("low"))
        ...
        events shouldContainExactly listOf("high:dispose", "high:initialize")
        engine.activeRuleFor(viewer, target).shouldNotBeNull().entryId shouldBe high.entryId
    }

    test("a restart asked for by a rule that no longer drives the pair does nothing") {
        val low = TestRuler(priority = 10)
        val high = TestRuler(priority = 20)
        low.apply(viewer, target, effect("low"))
        engine.awaitLifecycles()
        val stale = engine.activeRuleFor(viewer, target).shouldNotBeNull()
        high.apply(viewer, target, effect("high"))
        engine.awaitLifecycles()
        events.clear()

        engine.restartEffectNow(stale)
        engine.awaitLifecycles()
