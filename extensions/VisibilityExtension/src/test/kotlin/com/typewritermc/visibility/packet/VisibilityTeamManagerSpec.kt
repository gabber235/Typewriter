...
class VisibilityTeamManagerSpec : FunSpec({
    ...
    test("glow gives the pair its own team with the color") {
        val glow = TeamContribution(TeamContributionKind.GLOW, color = NamedTextColor.RED)
        val teams = compose("Viewer", listOf(pair(1, "Target", glow)))
    ...
    test("hidden nametag forces the name tag off") {
        val hidden = TeamContribution(TeamContributionKind.NAMETAG_HIDDEN, hidesNametag = true)
        val teams = compose("Viewer", listOf(pair(1, "Target", hidden)))

        teams[0].nameTagVisibility shouldBe WrapperPlayServerTeams.NameTagVisibility.NEVER
    }

    test("ghost puts the viewer and the target in the shared ghost team") {
        val ghost = TeamContribution(TeamContributionKind.GHOST, friendlyInvisible = true)
        val teams = compose("Viewer", listOf(pair(1, "Target", ghost)))
    ...
    test("ghost on a self pair does not duplicate the member") {
        val ghost = TeamContribution(TeamContributionKind.GHOST, friendlyInvisible = true)
        val teams = compose("Target", listOf(pair(1, "Target", ghost)))
    ...
    test("a contributed collision rule wins over the target's real team") {
        val teams = compose(
            "Viewer",
            listOf(
                pair(
                    ...
                    realTeam = teamInfo(collisionRule = WrapperPlayServerTeams.CollisionRule.ALWAYS),
                ),
            ),
        )
    ...
    test("the ghost team takes the collision rule a ghost pair contributes over the viewer's own") {
        val teams = compose(
            "Viewer",
            listOf(
                pair(
            ...
            viewerTeam = teamInfo(collisionRule = WrapperPlayServerTeams.CollisionRule.ALWAYS),
        )
    ...
    test("a pair's own team starts from a contributed base team instead of the target's real team") {
        val base = teamInfo(
            nameTagVisibility = WrapperPlayServerTeams.NameTagVisibility.NEVER,
            collisionRule = WrapperPlayServerTeams.CollisionRule.NEVER,
        )
        val teams = compose(
            ...
            listOf(
                pair(
                    ...
                    "fake-member",
                    TeamContribution(TeamContributionKind.GLOW, color = NamedTextColor.RED),
                    TeamContribution(TeamContributionKind.DISGUISE, baseTeam = base),
                    realTeam = teamInfo(color = NamedTextColor.BLUE, prefix = Component.text("[Admin] ")),
                ),
            ),
        )

        teams[0].color shouldBe NamedTextColor.RED
        teams[0].prefix shouldBe Component.empty()
        teams[0].nameTagVisibility shouldBe WrapperPlayServerTeams.NameTagVisibility.NEVER
        teams[0].collisionRule shouldBe WrapperPlayServerTeams.CollisionRule.NEVER
    }

    test("a base team one ghost brings does not hide the nametags of the viewer's other ghosts") {
        val ghost = TeamContribution(TeamContributionKind.GHOST, friendlyInvisible = true)
        val base = teamInfo(nameTagVisibility = WrapperPlayServerTeams.NameTagVisibility.NEVER)
        val teams = compose(
            "Viewer",
            listOf(
                pair(1, "fake-member", ghost, TeamContribution(TeamContributionKind.DISGUISE, baseTeam = base)),
                pair(2, "Other", ghost),
            ),
        )
