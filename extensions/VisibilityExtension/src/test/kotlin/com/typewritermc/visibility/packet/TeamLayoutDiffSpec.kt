...
import com.github.retrooper.packetevents.wrapper.play.server.WrapperPlayServerTeams.OptionData.FRIENDLY_CAN_SEE_INVISIBLE
...
class TeamLayoutDiffSpec : FunSpec({
    ...
    test("a member moving from a pair team to the ghost team is never left without a team") {
        val packets = diffTeamLayout(
            ...
            layout(spec(GHOST_TEAM_NAME, "Target", "Viewer", option = FRIENDLY_CAN_SEE_INVISIBLE)),
        )

        packets shouldContainExactly listOf(
            TeamPacket.Remove("twv_p_1"),
            TeamPacket.Create(spec(GHOST_TEAM_NAME, "Target", "Viewer", option = FRIENDLY_CAN_SEE_INVISIBLE)),
        )
    }
