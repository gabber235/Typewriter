...
class VisibilityHideRegistrySpec : FunSpec({
    ...
    test("a disguise the player sees on themselves is recorded without hiding them") {
        val player = server.addPlayer("Player")

        registry.hide(player, player, VisibleReplacement(42, "fake-member"))

        registry.replacementFor(player.uniqueId, player.uniqueId) shouldBe VisibleReplacement(42, "fake-member")
        hides.ownerCount(player.uniqueId, player.uniqueId) shouldBe 0
        player.canSee(player) shouldBe true

        registry.show(player.uniqueId, player.uniqueId)

        registry.replacementFor(player.uniqueId, player.uniqueId) shouldBe null
    }
