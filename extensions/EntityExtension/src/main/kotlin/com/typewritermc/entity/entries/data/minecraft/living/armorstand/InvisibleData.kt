...
@Entry("invisible_data", "Whether the entity itself is invisible", Colors.RED, "mdi:mirror-variant")
@Tags("invisible_data")
class InvisibleData(
...
) : GenericEntityData<InvisibleProperty> {
...
data class InvisibleProperty(val invisible: Boolean) : EntityProperty {
    companion object :
        SinglePropertyCollectorSupplier<InvisibleProperty>(InvisibleProperty::class, InvisibleProperty(false))
}

fun applyInvisibleData(entity: WrapperEntity, property: InvisibleProperty) {
    entity.metas {
        meta<EntityMeta> { isInvisible = property.invisible }
