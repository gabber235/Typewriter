...
<div class="mx-auto h-full px-24 py-8 dark:bg-[#101013]">
	<div class="flex flex-col gap-8 lg:flex-row">
		...
		<div class="flex-1 lg:w-2/3">
			<div class="mb-8 flex items-center gap-6">
				<div class="flex-shrink-0">
					<img
						...
						alt="Product banner"
						class="h-36 w-36 rounded-lg shadow-md dark:shadow-[#1E1E24]"
					/>
				</div>
				<div class="flex-1">
					<div class="mb-2 flex items-center justify-between gap-4">
						<div>
							<h1 class="mb-2 text-3xl font-bold dark:text-[#EEF9FC]">{product.name}</h1>
						...
						<div class="flex items-center gap-2">
							...
							<Button variant="default" class="gap-2" style="background-color: {product.color}">
								...
								{product.price === 0 ? "Download" : `${product.price} ${product.currency}`}
							</Button>
						</div>
					</div>
					<div class="flex items-center gap-2">
			...
			<Tabs value="description" class="w-full">
				<TabsList class="rounded-lg bg-[#1E1E24] p-1">
					<TabsTrigger
						value="description"
						class="rounded px-4 py-2 text-sm font-medium text-gray-400 transition-colors hover:text-gray-300 data-[state=active]:bg-[#2A2A30] data-[state=active]:text-white"
					>
						Description
					</TabsTrigger>
					<TabsTrigger
				...
				<TabsContent
					value="description"
					class="prose max-w-none dark:prose-invert dark:text-[#EEF9FC]"
				>
					<p
						class="flex h-10 w-full items-center justify-start rounded-md bg-white px-4 py-6 text-muted-foreground dark:bg-zinc-900 dark:text-gray-300"
					>
						{product.shortDescription}
					</p>
				</TabsContent>
				<TabsContent value="gallery" class="prose max-w-none dark:prose-invert dark:text-[#EEF9FC]">
					<p
						class="flex h-10 w-full items-center justify-start rounded-md bg-white px-4 py-6 text-muted-foreground dark:bg-zinc-900 dark:text-gray-300"
					>
				...
				>
					<p
						class="flex h-10 w-full items-center justify-start rounded-md bg-white px-4 py-6 text-muted-foreground dark:bg-zinc-900 dark:text-gray-300"
					>
						Version history and changelog will be displayed here
					</p>
				</TabsContent>
				{#if page.params.type === "extension"}
					<TabsContent
					...
					>
						<div class="space-y-6">
							...
							<div class="flex items-center gap-4">
								...
								<DropdownMenu>
									<DropdownMenuTrigger
										class="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm dark:border-gray-600 dark:bg-zinc-900 dark:text-gray-300"
									>
										{selectedType || "Type"}
									</DropdownMenuTrigger>
									<DropdownMenuContent>
										<DropdownMenuItem on:click={() => (selectedType = "")}
											...
											<Sub>
												...
												<SubContent>
													{#each [...subTypes] as subType}
														<DropdownMenuItem
															on:click={() => (selectedType = `${category}.${subType}`)}
														>
															{subType}
														</DropdownMenuItem>
													{/each}
		...
		<!-- Sidebar -->
		<div class="space-y-4 lg:w-1/3 dark:text-[#EEF9FC]">
			<CreatorWidget avatarUrl={product.creator.avatarUrl} name={product.creator.name} />
			<StatsWidget
				views={product.views}
			...
			/>
			<DateInfoWidget publishDate={product.publishDate} updateDate={product.updateDate} />
			<LicenseWidget type={product.license.type} />
			<ItemListWidget title="Tags" items={product.tags} />
			<ItemListWidget title="Platforms" items={product.platforms} />
		</div>
	</div>
</div>
