...
export function initVideoPlayer(container: HTMLElement) {
  ...
  const progressTrack =
    container.querySelector<HTMLElement>("[data-progress-track]");
  const progressFill =
    container.querySelector<HTMLElement>("[data-progress-fill]");
  const progressThumb =
    container.querySelector<HTMLElement>("[data-progress-thumb]");
  const playBtn = container.querySelector<HTMLElement>("[data-play-btn]");
  const playBtnIcon =
    container.querySelector<HTMLElement>("[data-play-icon]");
  const volumeBtn = container.querySelector<HTMLElement>("[data-volume-btn]");
  const volumeBtnIcon =
    container.querySelector<HTMLElement>("[data-volume-icon]");
  const volumeTrack =
    container.querySelector<HTMLElement>("[data-volume-track]");
  const volumeFill =
    container.querySelector<HTMLElement>("[data-volume-fill]");
  const fullscreenBtn =
    container.querySelector<HTMLElement>("[data-fullscreen-btn]");
  const fullscreenIcon_ =
    container.querySelector<HTMLElement>("[data-fullscreen-icon]");
  const timestamp = container.querySelector<HTMLElement>("[data-timestamp]");

  const centerIcon =
    container.querySelector<HTMLElement>("[data-center-icon]");
  ...
  const chapterLabelsContainer =
    container.querySelector<HTMLElement>("[data-chapter-labels]");
  ...
  function renderChapterDividers() {
    ...
    parsedChapters = [
      ...(JSON.parse(raw) as ChapterEntry[]),
    ...
    const allPoints = [{ time: 0, label: undefined as string | undefined }, ...parsedChapters].sort(
      (a, b) => a.time - b.time,
    );

    for (let i = 0; i < allPoints.length; i++) {
      const ch = allPoints[i];
      const nextTime = i < allPoints.length - 1 ? allPoints[i + 1].time : video.duration;
