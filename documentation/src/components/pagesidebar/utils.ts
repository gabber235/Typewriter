...
export function getRootMargin(
	mobileTocSelector = "summary",
): `-${number}px 0% ${number}px` {
	const navBarHeight =
		document.querySelector("header")?.getBoundingClientRect().height || 0;
	const mobileTocHeight =
		document.querySelector(mobileTocSelector)?.getBoundingClientRect().height ||
		0;
...
export function findParentLinks(
	currentLink: HTMLAnchorElement,
	_tocElement: HTMLElement,
): HTMLAnchorElement[] {
	...
	while (listItem) {
		...
		const parentLink = parentList.querySelector<HTMLAnchorElement>(
			":scope > div > a, :scope > a",
		);
