...
export class StarlightTOC extends HTMLElement {
	protected _current: HTMLAnchorElement | null =
		this.querySelector<HTMLAnchorElement>('a[aria-current="true"]');
	...
	protected progressText =
		this.querySelector<HTMLSpanElement>(".progress-text");
	...
	protected init(): void {
		...
		this.headings = [
			...document.querySelectorAll<HTMLHeadingElement>(this.tocHeadingSelector),
		];
	...
	protected calculateHeadingBasedProgress(): number {
		...
		const nextHeadingPos =
			currentIndex < headingPositions.length - 1
		...
		const sectionProgress =
			sectionLength > 0 ? positionInSection / sectionLength : 1;
		...
		for (let i = 0; i < headingPositions.length; i++) {
			...
			const sectionEnd =
				i < headingPositions.length - 1 ? headingPositions[i + 1] : contentEnd;
	...
	protected toggleParentHighlights(
	...
	protected scrollToActiveLink(link: HTMLAnchorElement): void {
		...
		const tocScrollContainer = this.querySelector<HTMLElement>(
		...
		const targetScroll =
			currentScroll + relativeTop - containerHeight / 2 + linkHeight / 2;
