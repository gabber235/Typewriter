...
export function initTabs(container: HTMLElement): void {
  ...
  const defaultTab = clamp(Number(container.dataset.defaultTab) || 0, 0, panels.length - 1);
  ...
  const ctx: TabContext = { container, tablist, pill, buttons, panels, labels, syncKey };
  ...
  tabActivators.set(container, (idx: number) => applyTab(ctx, state, idx, false));
...
function applyTab(ctx: TabContext, state: TabState, index: number, focus: boolean): void {
  ...
  for (let i = 0; i < ctx.buttons.length; i++) {
    ...
    ctx.buttons[i].className = `${s.button} ${isActive ? s.buttonActive : s.buttonInactive}`;

    ctx.panels[i].hidden = !isActive;
    ctx.panels[i].className = `${s.panel} ${isActive ? s.panelActive : s.panelHidden}`;
  }
...
function findPanelIndexByLabel(panels: NodeListOf<HTMLElement>, label: string): number {
...
function resolveInitialTab(labels: string[], defaultTab: number, syncKey: string | null): number {
  ...
  for (const other of group) {
    ...
    for (let i = 0; i < otherPanels.length; i++) {
      ...
      const idx = labels.findIndex((l) => l === matchLabel);
      if (idx !== -1) return idx;
    }
  }
