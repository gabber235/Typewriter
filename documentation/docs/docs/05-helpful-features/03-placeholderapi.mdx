...
## Custom placeholders
Typewriter also has 5 custom placeholders, these are:
...
- `%typewriter_in_dialogue%`: Returns `0` if not in a dialogue, `1` if in a dialogue.
- `%typewriter_in_cinematic%`: Returns `0` if not in cinematic mode, `1` if in cinematic mode.
