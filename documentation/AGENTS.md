...
- Content in `src/content/docs/`, split into two sections (`docs/` and `develop/`) with separate sidebars — see [src/content/AGENTS.md](src/content/AGENTS.md) for markdown rules
- Sections are defined in `src/sections.ts`; `src/route-middleware.ts` (Starlight `routeMiddleware`) scopes the autogenerated sidebar and prev/next pagination to the current section
