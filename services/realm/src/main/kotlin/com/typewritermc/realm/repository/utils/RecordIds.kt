...
internal fun RecordId.invalidRecordId(expectedTable: String): InvalidRecordIdError? =
    listOf(this).invalidRecordId(expectedTable)
...
internal fun com.surrealdb.RecordId.toSkirRecordId(): RecordId = RecordId(
    table = table,
    key = id.toSkirRecordIdKey(),
)

private fun Id.toSkirRecordIdKey(): RecordIdKey = when {
    isLong -> RecordIdKey.NumberWrapper(long)
    isString -> RecordIdKey.StringWrapper(string)
    isUuid -> RecordIdKey.UuidWrapper(uuid.toString())
    isArray -> RecordIdKey.ArrayWrapper(array.map(Value::toSkirRecordIdValue))
    isObject -> RecordIdKey.ObjectWrapper(
...
private fun Value.toSkirRecordIdValue(): RecordIdValue = when {
    isNull || isNone -> RecordIdValue.NULL
    isBoolean -> RecordIdValue.BooleanWrapper(boolean)
    isLong -> RecordIdValue.NumberWrapper(long)
    isDouble -> RecordIdValue.FloatWrapper(double)
    isString -> RecordIdValue.StringWrapper(string)
    isArray -> RecordIdValue.ArrayWrapper(array.map(Value::toSkirRecordIdValue))
    isObject -> RecordIdValue.ObjectWrapper(
...
internal fun RecordId.toSurrealRecordId(): com.surrealdb.RecordId = when(this.key) {
    is RecordIdKey.NumberWrapper -> com.surrealdb.RecordId(table, (this.key as RecordIdKey.NumberWrapper).value)
    is RecordIdKey.StringWrapper -> com.surrealdb.RecordId(table, (this.key as RecordIdKey.StringWrapper).value)
    is RecordIdKey.UuidWrapper -> com.surrealdb.RecordId(table, UUID.fromString((this.key as RecordIdKey.UuidWrapper).value))
    is RecordIdKey.ArrayWrapper -> com.surrealdb.RecordId(table, com.surrealdb.Array.fromList((this.key as RecordIdKey.ArrayWrapper).value.map(RecordIdValue::databaseValue)))
    is RecordIdKey.ObjectWrapper -> throw UnsupportedOperationException("Objects are not supported in the java sdk yet. Maybe in the kotlin sdk in the future?")
    is RecordIdKey.Unknown -> error("Unknown record id key")
}

private fun RecordIdValue.databaseValue(): Any? = when (this) {
