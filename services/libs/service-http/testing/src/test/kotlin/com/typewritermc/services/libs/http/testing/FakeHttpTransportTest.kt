...
val FakeHttpTransportTest by testSuite {
    test("records immutable ordered redacted actions and scripted failures") {
        ...
        val request = HttpRequest(HttpOperation("fake.call"), ErrorSlug.of("fake-failed"), HttpMethod.POST,
            URI("https://example.test/path?token=secret"), HttpHeaders.of("authorization" to "secret"), body)
        ...
        fake.actions.first().headers.first("authorization") shouldBe "secret"
    ...
    test("fails when the script is exhausted") {
        ...
        val request = HttpRequest(HttpOperation("fake.call"), ErrorSlug.of("fake-failed"), HttpMethod.GET,
