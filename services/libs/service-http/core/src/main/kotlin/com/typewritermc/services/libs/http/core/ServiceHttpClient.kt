...
class ServiceHttpClient(
    ...
    suspend fun execute(request: HttpRequest): HttpResult {
        ...
        return telemetry.mainSpan(request.operation.value, request.failureSlug, SpanKind.CLIENT, attributes = attributes) { span ->
            try {
                val result = request.maximumRequestBytes?.takeIf { bodySize > it }?.let {
                ...
                span.annotate {
                    when (result) {
                        is HttpResult.Success -> {
                            ...
                            attribute("http.response.body.size", result.response.body.size.toLong())
                        }
    ...
    private fun propagatedRequest(request: HttpRequest, body: ByteArray): HttpRequest {
        val setter = HeaderSetter(propagators.textMapPropagator.fields().fold(request.headers) { headers, field ->
            headers.remove(field)
        })
        propagators.textMapPropagator.inject(Context.current(), setter, HeaderSetter.Setter)
        return HttpRequest(request.operation, request.failureSlug, request.method, request.uri, setter.headers,
            body, request.timeout, request.maximumRequestBytes, request.maximumResponseBytes)
    }
}

private val HttpError.category: String get() = when (this) {
...
private class HeaderSetter(var headers: HttpHeaders) {
    object Setter : TextMapSetter<HeaderSetter> {
        override fun set(carrier: HeaderSetter?, key: String, value: String) {
