...
class HttpHeaders private constructor(private val entries: List<Pair<String, String>>) : Iterable<Pair<String, String>> {
    ...
    fun add(name: String, value: String): HttpHeaders = of(entries + (name to value))
    fun set(name: String, value: String): HttpHeaders = of(entries.filterNot { it.first.equals(name, true) } + (name to value))
    ...
    companion object {
        ...
        fun of(entries: Iterable<Pair<String, String>>): HttpHeaders {
            ...
            copy.forEach { (name, value) ->
                require(name.isNotEmpty() && name.all { it.code in 33..126 && it !in "():<>@,;\\\"/[]?={} \t" }) { "Invalid HTTP header name" }
...
@JvmInline value class HttpOperation(val value: String) {
    init { require(Regex("[a-z0-9]+(?:[.-][a-z0-9]+)*").matches(value)) { "Invalid HTTP operation" } }
}
...
class HttpResponse(val statusCode: Int, val headers: HttpHeaders, body: ByteArray) {
    private val bodyBytes = body.copyOf()
    val body: ByteArray get() = bodyBytes.copyOf()
...
sealed interface HttpError {
    data class RequestTooLarge(val maximumBytes: Long, val actualBytes: Long) : HttpError
    data class ResponseTooLarge(val maximumBytes: Long) : HttpError
    data object Timeout : HttpError
    data object Unavailable : HttpError
    data class Transport(val type: String) : HttpError {
        override fun toString(): String = "HttpError.Transport"
    }
    data class Invalid(val reason: String) : HttpError {
...
sealed interface HttpResult {
    ...
    class Failure(val error: HttpError) : HttpResult {
...
fun interface HttpTransport { suspend fun execute(request: HttpRequest): HttpResult }
