...
val HttpTypesTest by testSuite {
    ...
    test("requests reject restricted headers case insensitively") {
        listOf("connection", "Content-Length", "EXPECT", "Host", "upgrade").forEach { name ->
            shouldThrow<IllegalArgumentException> {
                HttpRequest(HttpOperation("test"), ErrorSlug.of("test"), HttpMethod.GET, URI("https://example.test"), HttpHeaders.of(name to "value"))
            }
        }
    }
    test("request and response bodies are defensively copied") {
        val source = byteArrayOf(1, 2)
        val request = HttpRequest(HttpOperation("test.get"), ErrorSlug.of("test-failure"), HttpMethod.POST, URI("https://example.test"), body = source)
        source[0] = 9
        request.body[1] = 9
    ...
    test("diagnostics redact URI query headers body and error details") {
        val request = HttpRequest(HttpOperation("test"), ErrorSlug.of("test"), HttpMethod.POST,
            URI("https://example.test/path?token=query-secret"), HttpHeaders.of("authorization" to "header-secret"), "body-secret".toByteArray())
        val diagnostics = listOf(request, HttpResponse(200, request.headers, "response-secret".toByteArray()),
            HttpError.Invalid("token-secret"), HttpError.Transport("secret.Transport"), HttpResult.Failure(HttpError.Invalid("secret"))).joinToString()
    ...
    test("requests validate URI body timeout and limits") {
        shouldThrow<IllegalArgumentException> { HttpRequest(HttpOperation("test"), ErrorSlug.of("test"), HttpMethod.GET, URI("ftp://example.test")) }
        shouldThrow<IllegalArgumentException> { HttpRequest(HttpOperation("test"), ErrorSlug.of("test"), HttpMethod.GET, URI("https://example.test"), body = byteArrayOf(1)) }
        shouldThrow<IllegalArgumentException> { HttpRequest(HttpOperation("test"), ErrorSlug.of("test"), HttpMethod.GET, URI("https://example.test"), maximumResponseBytes = 0) }
    }
}
