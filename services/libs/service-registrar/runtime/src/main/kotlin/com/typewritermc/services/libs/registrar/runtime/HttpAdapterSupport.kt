...
internal val skirRequestHeaders = HttpHeaders.of(
    "Content-Type" to SKIR_MEDIA_TYPE,
    "Accept" to SKIR_MEDIA_TYPE,
    "X-Typewriter-Format" to "skir-binary",
)
internal val skirGetHeaders = HttpHeaders.of(
    "Accept" to SKIR_MEDIA_TYPE,
    "X-Typewriter-Format" to "skir-binary",
)

internal fun HttpHeaders.hasSkirMediaType(): Boolean = first("Content-Type")
    ?.substringBefore(';')
    ?.trim()
