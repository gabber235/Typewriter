...
import skirout.service.v1.status.*
import kotlin.time.TimeSource

@JvmInline
value class ServiceAddress(val serviceId: String) {
    init { require(serviceId.isNotBlank()) }
}

private fun serviceAddress(pattern: String): AddressTemplate<ServiceAddress> = addressTemplate(
    pattern,
...
private val statusPolicy = ResponsePolicy<GetServiceStatusResponse>(
    ...
    ResponseClassifier { response -> when (response.kind) {
        GetServiceStatusResponse.Kind.STATUS_WRAPPER -> classification(ResponseOutcome.SUCCESS, "status")
        GetServiceStatusResponse.Kind.SERVICE_NOT_FOUND_ERROR_WRAPPER -> classification(ResponseOutcome.DOMAIN_ERROR, "service-not-found")
        GetServiceStatusResponse.Kind.INTERNAL_ERROR_WRAPPER -> classification(ResponseOutcome.INTERNAL_ERROR, "internal-error")
        GetServiceStatusResponse.Kind.UNKNOWN -> classification(ResponseOutcome.DOMAIN_ERROR, "unknown")
    } },
)
private val boundClassifier = ResponseClassifier<ServiceBoundNotification> { classification(ResponseOutcome.SUCCESS, "bound") }
private fun classification(outcome: ResponseOutcome, variant: String) = ResponseClassification(outcome, ResponseVariant.of(variant))
private val statusContract = skirUnaryContract(GetServiceStatus, OperationName.of("registrar.status"), serviceStatusAddress, statusPolicy, ErrorSlug.of("registrar-status-failed"))
private val bindingContract = skirWatchContract(GetServiceStatus, ServiceBoundNotification.serializer, OperationName.of("registrar.binding"), serviceStatusAddress, serviceBoundAddress, statusPolicy, boundClassifier, ErrorSlug.of("registrar-binding-failed"))
private val heartbeatContract = EventContract(OperationName.of("registrar.heartbeat"), serviceHeartbeatAddress, ServiceHeartbeatNotification.serializer.asPayloadCodec(), ErrorSlug.of("registrar-heartbeat-failed"))
private val shutdownContract = EventContract(OperationName.of("registrar.shutdown"), serviceShutdownAddress, ServiceShutdownNotification.serializer.asPayloadCodec(), ErrorSlug.of("registrar-shutdown-failed"))
...
private class ProductionNatsLifecycle(private val connection: NatsConnection) : NatsLifecycle {
...
private fun NatsConnectionState.toRuntimeConnectivity(): RuntimeConnectivity = when (this) {
...
internal class TypewriterRegistrarRuntime(
    ...
    override fun watchBinding(): Flow<RuntimeResult<BindingObservation>> = communicator
        .watch(bindingContract, service, GetServiceStatusRequest())
        .map { result -> when (result) {
            is CommunicationResult.Failure -> messaging(MessagingOperation.BINDING_WATCH)
            is CommunicationResult.Success -> when (val message = result.value) {
                is WatchMessage.Initial -> mapStatus(message.value, MessagingOperation.BINDING_WATCH)
                    .map { BindingObservation.Initial(it) }
                is WatchMessage.Update -> mapBound(message.value).map { BindingObservation.Bound(it) }
            }
        } }

    override suspend fun sendHeartbeat() =
        publish(heartbeatContract, ServiceHeartbeatNotification(), MessagingOperation.HEARTBEAT)

    override suspend fun sendShutdown() =
        publish(shutdownContract, ServiceShutdownNotification(), MessagingOperation.SHUTDOWN)
    ...
    private suspend fun lifecycle(operation: MessagingOperation, action: suspend () -> NatsLifecycleResult): RuntimeResult<Unit> {
        val result = try { action() } catch (failure: RegistrarAuthenticationException) { return RuntimeResult.Failure(failure.failure) }
    ...
    private suspend fun <E : Any> publish(contract: EventContract<ServiceAddress, E>, event: E, operation: MessagingOperation) =
...
): RuntimeResult<BindingStatus> = when (response.kind) {
    GetServiceStatusResponse.Kind.SERVICE_NOT_FOUND_ERROR_WRAPPER ->
        RuntimeResult.Failure(RegistrarFailure.ServiceNotFound)
    GetServiceStatusResponse.Kind.INTERNAL_ERROR_WRAPPER -> messaging(operation)
...
internal fun mapBinding(binding: ServiceBinding): RuntimeResult<BindingStatus> = when (binding.kind) {
    ServiceBinding.Kind.UNKNOWN -> RuntimeResult.Failure(RegistrarFailure.ProtocolIncompatible("service-binding", "unknown"))
    ...
    ServiceBinding.Kind.UNBOUND_WRAPPER -> {
        ...
        when {
            token == null -> RuntimeResult.Success(BindingStatus.Unbound(null))
            token.isBlank() -> RuntimeResult.Failure(
                RegistrarFailure.ProtocolIncompatible("service-binding", "blank-registration-token")
            )
            else -> RuntimeResult.Success(BindingStatus.Unbound(RegistrationToken(token)))
        }
    }
}
...
private fun mapBound(id: String, name: String?): RuntimeResult<OrganizationBinding> {
...
private fun <A, B> RuntimeResult<A>.map(transform: (A) -> B): RuntimeResult<B> = when (this) {
...
internal class RegistrarAuthenticationException(val failure: RegistrarFailure) : RuntimeException(null, null, false, false)
...
class TypewriterRegistrarRuntimeFactory(
    ...
    override suspend fun create(credentials: IdentityCredentials, progress: RuntimeSetupProgressSink): RuntimeCreateResult {
        val exchanger = AuthentikTokenExchanger(httpClient, configuration.oauthTokenUri, configuration.oauthClientId, configuration.oauthScopes)
        val access = AccessTokenCache(credentials, exchanger, clock, configuration.accessTokenRefreshSkew)
        val sentinel = SentinelCache(TypewriterSentinelProvider(httpClient, configuration.sentinelCredentialsUri), configuration.sentinelRefreshAfter, configuration.sentinelMaximumStaleness, clock)
        progress.report(RuntimeSetupProgress.ACQUIRING_ACCESS_TOKEN)
        when (val result = access.get()) { is AccessTokenResult.Failure -> return RuntimeCreateResult.Failure(result.failure); else -> Unit }
        progress.report(RuntimeSetupProgress.ACQUIRING_SENTINEL_CREDENTIALS)
        when (val result = sentinel.get()) { is SentinelResult.Failure -> return RuntimeCreateResult.Failure(result.failure); else -> Unit }
        val connection = NatsConnection({ serviceNatsConfiguration(configuration, credentials) }, serviceNatsAuthenticationProvider(access, sentinel, credentials))
        ...
        return RuntimeCreateResult.Success(TypewriterRegistrarRuntime(communicator, ServiceAddress(credentials.identity.serviceId), ProductionNatsLifecycle(connection), access, sentinel))
    }
}
