...
import com.typewritermc.services.libs.registrar.testing.*
...
val ServiceRegistrarLifecycleTest by testSuite {
    test("concurrent start is idempotent and loads once") { runTest {
        fixture(this).use { f ->
            f.readyScript()
    ...
    test("stop before start is successful and restart is rejected") { runTest {
        fixture(this).use { f ->
            f.registrar.stop() shouldBe RegistrarStopResult.Success
            f.registrar.start().shouldBeInstanceOf<RegistrarResult.Failure>()
        }
    } }
    test("repeated stop returns the exact idempotent result") { runTest {
        fixture(this).use { f ->
            val first = f.registrar.stop()
            (f.registrar.stop() === first) shouldBe true
        }
    } }
    test("stored credentials skip identity issuance") { runTest {
    ...
    test("missing credentials issue and store once with immutable reference evidence") { runTest {
        fixture(this).use { f ->
            val value = credentials(); f.issuer.enqueue(IdentityIssueResult.Success(value)); f.readyScript()
            f.registrar.start(); runCurrent()
            f.ledger.actions.count { it is RegistrarAction.IssueIdentity } shouldBe 1
            val store = f.ledger.actions.filterIsInstance<RegistrarAction.StoreCredentials>().single()
            val create = f.ledger.actions.filterIsInstance<RegistrarAction.CreateRuntime>().single()
            store.referenceId shouldBe create.credentialReferenceId
        }
    } }
    test("ambiguous issuance is terminal and does not auto retry") { runTest {
        fixture(this).use { f ->
            f.issuer.enqueue(IdentityIssueResult.Failure(IdentityIssueError.Unavailable(true)))
            f.registrar.start(); runCurrent(); advanceTimeBy(10_000); runCurrent()
            (f.registrar.states.value.state as RegistrarState.Failed).identityOutcomeMayBeAmbiguous shouldBe true
            f.ledger.actions.count { it is RegistrarAction.IssueIdentity } shouldBe 1
        }
    } }
    test("explicit retry issues after a terminal issue failure") { runTest {
        fixture(this).use { f ->
            f.issuer.enqueue(IdentityIssueResult.Failure(IdentityIssueError.Rejected(IdentityRejectionReason.MALFORMED_REQUEST)))
            f.issuer.enqueue(IdentityIssueResult.Success(credentials())); f.readyScript()
            f.registrar.start(); runCurrent(); f.registrar.retry(); runCurrent()
            f.ledger.actions.count { it is RegistrarAction.IssueIdentity } shouldBe 2
        }
    } }
    test("terminal store failure retains exact credentials for explicit retry") { runTest {
        fixture(this).use { f ->
            val value = credentials(); f.issuer.enqueue(IdentityIssueResult.Success(value))
            f.storage.enqueueStore(CredentialStoreResult.Failure(CredentialStorageError.Corrupt("bad")))
            f.storage.enqueueStore(CredentialStoreResult.Success); f.readyScript()
            f.registrar.start(); runCurrent(); f.registrar.retry(); runCurrent()
            f.ledger.actions.filterIsInstance<RegistrarAction.StoreCredentials>().map { it.referenceId }.distinct().size shouldBe 1
            f.ledger.actions.count { it is RegistrarAction.IssueIdentity } shouldBe 1
        }
    } }
    test("recoverable store failure retains credentials and grows storage backoff") { runTest {
        fixture(this).use { f ->
            ...
            f.storage.enqueueStore(CredentialStoreResult.Success); f.readyScript(); f.registrar.start(); runCurrent()
            f.ledger.actions.filterIsInstance<RegistrarAction.Delay>().map { it.duration } shouldBe listOf(1.seconds, 2.seconds)
        }
    } }
    test("corrupt load is terminal while unavailable load retries") { runTest {
        fixture(this, CredentialLoadResult.Failure(CredentialStorageError.Corrupt("bad"))).use { f ->
            f.registrar.start(); runCurrent(); f.registrar.states.value.state.shouldBeInstanceOf<RegistrarState.Failed>()
        }
        fixture(this, CredentialLoadResult.Failure(CredentialStorageError.Unavailable("down"))).use { f ->
            f.storage.enqueueLoad(CredentialLoadResult.Loaded(credentials())); f.readyScript(); f.registrar.start(); runCurrent()
            f.ledger.actions.any { it is RegistrarAction.Delay } shouldBe true
        }
    } }
    test("initial bound observation reaches ready and heartbeat is immediate") { runTest {
        fixture(this, CredentialLoadResult.Loaded(credentials())).use { f ->
            f.readyScript(); f.registrar.start(); runCurrent()
    ...
    test("initial unbound null token is defensively published") { runTest {
        fixture(this, CredentialLoadResult.Loaded(credentials())).use { f ->
            ...
            f.runtime.setConnectivity(RuntimeConnectivity.CONNECTED); f.factory.enqueue(RuntimeCreateResult.Success(f.runtime))
            f.registrar.start(); runCurrent()
            (f.registrar.states.value.state as RegistrarState.AwaitingBinding).registrationToken shouldBe null
        }
    } }
    test("binding notification reauthorizes and confirms binding") { runTest {
        fixture(this, CredentialLoadResult.Loaded(credentials())).use { f ->
    ...
    test("binding refresh queries while awaiting notification") { runTest {
        fixture(this, CredentialLoadResult.Loaded(credentials())).use { f ->
            f.runtime.enqueueWatch(); f.runtime.enqueueQuery(RuntimeResult.Success(BindingStatus.Unbound(RegistrationToken("next"))))
            f.runtime.setConnectivity(RuntimeConnectivity.CONNECTED); f.factory.enqueue(RuntimeCreateResult.Success(f.runtime))
            f.registrar.start(); runCurrent(); advanceTimeBy(101); runCurrent()
            f.ledger.actions.any { it == RegistrarAction.QueryBinding } shouldBe true
        }
    } }
    test("heartbeat is periodic and no binding calls occur after ready") { runTest {
        fixture(this, CredentialLoadResult.Loaded(credentials())).use { f ->
            f.readyScript(); f.registrar.start(); runCurrent(); val bindingCalls = f.bindingCallCount
    ...
    test("connectivity recovery increments generation") { runTest {
        fixture(this, CredentialLoadResult.Loaded(credentials())).use { f ->
    ...
    test("multiple awaiters share session and cancelling one does not cancel supervisor") { runTest {
        fixture(this, CredentialLoadResult.Loaded(credentials())).use { f ->
            val cancelled = launch { f.registrar.awaitReady() }; val survivor = async { f.registrar.awaitReady() }
            cancelled.cancelAndJoin(); f.readyScript(); f.registrar.start(); runCurrent()
            survivor.await().shouldBeInstanceOf<RegistrarResult.Success<ReadySession>>()
            f.registrar.states.value.state.shouldBeInstanceOf<RegistrarState.Ready>()
        }
    } }
    test("await during degraded waits for recovery") { runTest {
        fixture(this, CredentialLoadResult.Loaded(credentials())).use { f ->
            f.runtime.enqueueHeartbeat(RuntimeResult.Failure(RegistrarFailure.Messaging(MessagingOperation.HEARTBEAT)))
            f.readyScript(); f.registrar.start(); runCurrent(); val waiter = async { f.registrar.awaitReady() }
            waiter.isCompleted shouldBe false; runCurrent(); waiter.await().shouldBeInstanceOf<RegistrarResult.Success<ReadySession>>()
        }
    } }
    test("stop orders shutdown before close and prevents further heartbeat") { runTest {
        fixture(this, CredentialLoadResult.Loaded(credentials())).use { f ->
            f.readyScript(); f.registrar.start(); runCurrent(); f.registrar.stop(); val count = f.ledger.actions.count { it == RegistrarAction.Heartbeat }
            advanceTimeBy(1_000); runCurrent(); f.ledger.actions.count { it == RegistrarAction.Heartbeat } shouldBe count
    ...
    test("stop aggregates shutdown and close failures") { runTest {
        fixture(this, CredentialLoadResult.Loaded(credentials())).use { f ->
    ...
    test("state sequences and retry attempts are monotonic") { runTest {
        fixture(this).use { f ->
            f.issuer.enqueue(IdentityIssueResult.Success(credentials())); f.storage.enqueueStore(CredentialStoreResult.Failure(CredentialStorageError.Unavailable("x"))); f.readyScript()
            val seen = mutableListOf<RegistrarSnapshot>(); val collect = launch { f.registrar.states.collect { seen += it } }
            f.registrar.start(); runCurrent(); collect.cancelAndJoin()
            seen.zipWithNext().all { (a, b) -> b.sequence > a.sequence && b.attempt >= a.attempt } shouldBe true
        }
    } }
    test("public diagnostics redact tokens and credential secrets") { runTest {
...
private class Fixture(
    ...
    val registrar = ServiceRegistrar(configuration(), registrarScope, storage, issuer, factory, harness.telemetry, FakeRetryRandom(listOf(.5), ledger), FakeRegistrarDelay(ledger = ledger))
    val bindingCallCount get() = ledger.actions.count { it is RegistrarAction.WatchBinding || it == RegistrarAction.QueryBinding || it == RegistrarAction.Reconnect }
    fun readyScript() {
    ...
    override fun close() { registrarScope.cancel(); harness.close() }
}

private fun fixture(scope: kotlinx.coroutines.CoroutineScope, initial: CredentialLoadResult = CredentialLoadResult.Missing) = Fixture(scope, initial)
...
private fun configuration() = RegistrarConfiguration(
    URI("https://example.test/issue"), URI("https://example.test/sentinel"), URI("https://example.test/token"), "client", setOf("openid"), URI("nats://example.test"), listOf(ServiceRole.Engine("1")),
    bindingRefreshInterval = 100.milliseconds, heartbeatInterval = 100.milliseconds, shutdownTimeout = 100.milliseconds,
