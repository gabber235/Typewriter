...
private val statusAddress = addressTemplate(
...
private val statusPolicy = ResponsePolicy<GetServiceStatusResponse>(
    ...
    classifier = { response ->
        when (response.kind) {
            ...
            GetServiceStatusResponse.Kind.SERVICE_NOT_FOUND_ERROR_WRAPPER -> ResponseClassification(
                ResponseOutcome.DOMAIN_ERROR,
                ResponseVariant.of("service-not-found")
            )

            GetServiceStatusResponse.Kind.STATUS_WRAPPER -> ResponseClassification(
                ResponseOutcome.SUCCESS,
                ResponseVariant.of("status")
            )

            GetServiceStatusResponse.Kind.UNKNOWN -> ResponseClassification(
                ResponseOutcome.DOMAIN_ERROR,
...
val SkirAdaptersTest by testSuite {
    ...
    test("method helper preserves explicit operation and serializers") {
        val contract = skirUnaryContract(
