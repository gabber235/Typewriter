...
class NatsMessageTransport(private val connection: NatsConnection) : MessageTransport {
...
private class NatsTransportSubscription(
    ...
    override val deliveries: Flow<TransportDelivery> = flow {
        var terminalFailure = false
        subscription.messages.map { message ->
...
private fun OutboundMessage.toNatsMessage() = NatsClientMessage(
...
private fun NatsClientMessage.toInboundMessage() = InboundMessage(
    address = MessageAddress.of(subject),
    payload = payload ?: byteArrayOf(),
    replyTo = replyTo?.let(MessageAddress::of),
    headers = headers.orEmpty().entries.fold(MessageHeaders.Empty) { result, (name, values) ->
        values.fold(result) { headers, value -> headers.plus(name, value) }
    },
)

private fun NatsClientMessage.statusError(): TransportError? = when {
...
private class NatsStatusException(status: Int, description: String?) :
    RuntimeException("NATS status $status${description?.let { ": $it" }.orEmpty()}")

private suspend fun <Value> transportCall(block: suspend () -> Value): TransportResult<Value> = try {
