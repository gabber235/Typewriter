...
use wasmcloud_utils::wasmcloud::messaging::{reply, types::BrokerMessage};

use crate::{typewriter, OrganizationRecord};

pub fn internal_error_data() -> Vec<u8> {
    typewriter::api::v1::CreateOrganizationResponse {
        result: Some(
            typewriter::api::v1::create_organization_response::Result::Error(
                typewriter::models::v1::Error {
                    code: "500".to_string(),
                    message: "Internal Server Error when creating organization".to_string(),
                    details: vec![],
                },
            ),
        ),
