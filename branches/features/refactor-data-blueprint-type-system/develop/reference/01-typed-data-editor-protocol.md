...
description: Semantics for nominal editor types, exact values, catalogues,
  conversions, bindings, presentations, and realm actions.
editUrl: true
head: []
template: doc
sidebar:
...
Every `PresentationNode` may declare `PresentationHeader` chrome. A header title is either a typed text expression or a complete presentation node. Rich title nodes provide their own visible and semantic content. A header may also provide a typed description expression, an optional initial expansion state, typed actions, and header or content padding overrides. Padding may apply one value to every side, symmetric horizontal and vertical values, or individual physical top, left, right, and bottom values. Omitted padding uses the renderer defaults. Explicit zero insets remove the corresponding padding. A null expansion state means the header stays inline. A true or false value makes it collapsible and selects its initial state.
