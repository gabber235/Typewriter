...
void main() {
  group("selector parsing", () {
    ...
    test("parses key value selector", () {
      final result = checkLex("title:test")
          .expectNoQuery()
          .expectExpression((expression) {
    ...
    test("enforces selector key case sensitivity", () {
      ...
      final invalid = checkLex("EXACT:ok")
          .expectQuery("EXACT:ok")
          .expectNoExpression()
          .done();
  ...
  group("leftover query", () {
    test("fails fast when no expression is present", () {
      checkLex("hello world")
          .expectQuery("hello world")
          .expectNoExpression()
          .done();
    });

    test("keeps prefix and suffix outside parsed expression", () {
      checkLex("prefix #a suffix")
          .expectQuery("prefix suffix")
          .expectExpression((expression) {
    ...
    test("keeps multiple free text chunks around parsed expression", () {
      checkLex("pre one #a post two")
          .expectQuery("pre one post two")
          .expectExpression((expression) {
            expression.isSelector(id: "tag", value: "a");
          })
          .done();
